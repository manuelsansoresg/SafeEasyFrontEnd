...
export function LocationProvider() {
  ...
  useEffect(() => {
            const debugLocation = process.env.NEXT_PUBLIC_MIDDLEWARE_DEBUG === "true";
            ...
            if (debugLocation && mwDebug) {
              if (process.env.NODE_ENV === "development") console.log("✅ Middleware está corriendo correctamente.");
              if (mwHeaders) {
                if (process.env.NODE_ENV === "development") console.log("🔍 Headers recibidos por Middleware:", JSON.parse(decodeURIComponent(mwHeaders as string)));
              }
            } else if (debugLocation) {
              ...
              try {
                  ...
                  try {
                    ...
                    if (cleanCity.includes('Ã')) {
                      ...
                      if (debugLocation) {
                        if (process.env.NODE_ENV === "development") console.log("No se pudo corregir encoding avanzado, usando valor original");
                      }
                  }

                  if (debugLocation) {
                    if (process.env.NODE_ENV === "development") console.log("📍 Ubicación recuperada (Original):", city);
                    if (process.env.NODE_ENV === "development") console.log("📍 Ubicación recuperada (Corregida):", cleanCity);
                  }
       ...
       if (debugLocation) {
         if (process.env.NODE_ENV === "development") console.log("⚠️ No se detectó ubicación en cookies cliente");
       }
       
       // Fallback for local development
       if (process.env.NODE_ENV === 'development') {
           if (debugLocation) {
             if (process.env.NODE_ENV === "development") console.log("🔧 Modo Desarrollo: Usando ubicación simulada (Mérida, MX)");
           }
