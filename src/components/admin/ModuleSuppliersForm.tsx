...
export function ModuleSuppliersForm({
  ...
  const [detail, setDetail] = useState<ModuleAdminDetail | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const [moduleDetail, supplierItems] = await Promise.all([
          moduleService.detail(moduleId, controller.signal),
          moduleService.suppliers("", controller.signal),
        ]);
        ...
        setSelectedIds(new Set(moduleDetail.allowed_supplier_ids || []));
      } catch (loadError) {
  ...
  async function save() {
    ...
    try {
      ...
      onSaved("Proveedores adicionales actualizados.");
      onClose();
    } catch (saveError) {
      ...
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-xs leading-5 text-gray-500">
            Estos proveedores podrán ver el módulo aunque su plan actual no esté entre los planes ligados al módulo. No sustituye los planes: funciona como una excepción adicional.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          {unknownSelectedIds.length ? (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
              Se conservan también los proveedores seleccionados que no aparecen en el listado actual: {unknownSelectedIds.map((id) => `#${id}`).join(", ")}.
            </div>
          ) : null}

          <div className="max-h-[48dvh] divide-y divide-gray-100 overflow-y-auto rounded-xl border border-gray-200">
            {filteredSuppliers.length === 0 ? (
              <p className="p-6 text-center text-sm text-gray-500">No se encontraron proveedores.</p>
            ) : (
              filteredSuppliers.map((supplier) => {
                const checked = selectedIds.has(supplier.id);

                return (
                  <label key={supplier.id} className="flex cursor-pointer items-start gap-3 p-4 transition hover:bg-gray-50">
                    <span
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block font-semibold text-gray-900">{supplierLabel(supplier)}</span>
